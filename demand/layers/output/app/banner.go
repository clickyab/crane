...
<div id="showb" class="adhere" onclick="showb()">
    ...
    <iframe style="margin:0;padding:0;border:none;" src="{{ .ShowURL }}" />
</div>
    {{ else }}
        <a onClick="onClickyabClicked('{{ .ClickURL }}',null,null)"><iframe style="margin:0;padding:0;border:none;" src="{{ .ShowURL }}" /></a>;
    {{ end }}
{{ end }}
...
//
func Render(closeClass, full string, finger, noAd bool, sdk int64, w io.Writer, seat entity.Seat) error {
	return inappTemplate.Execute(w, inappContext{
		ShowURL:       seat.ShowURL(),
		ClickURL:      seat.ClickURL(),
		ExtraStyle:    "",
		ImpID:         rand.Int(),
		Dynamic:       false,
		FatFinger:     finger,
