...
TAG registry.clickyab.ae/clickyab/{{ .App }}:A{{ .Version }}
PUSH registry.clickyab.ae/clickyab/{{ .App }}:A{{ .Version }}
...
do
   kubectl -n ${NAMESPACE} set image deployment  ${APP}-${WRK_TYP} ${APP}-${BRANCH}=registry.clickyab.ae/clickyab/${APP}_${BRANCH}:A${VERSION} --record
done
echo "..." >> ${OUT_LOG}
