...
// FakeChannel fake implemention of Channel interface
...
func (f *FakeChannel) Confirm(noWait bool) error {
	logrus.Debugf(
		`ok we confirm and puts this channel into confirm mode so that the client can ensure all
		publishing's have successfully been received by the server with no wait %v`,
		noWait,
	)
...
func (f *FakeChannel) NotifyPublish(confirm chan amqp.Confirmation) chan amqp.Confirmation {
	logrus.Debugf("registers a listener for reliable publishing.")
...
func (f *FakeChannel) Publish(exchange, key string, mandatory, immediate bool, msg amqp.Publishing) error {
	logrus.Debugf("publish a job to %s with key %s", exchange, key)
...
func (f *FakeChannel) Close() error {
	logrus.Debugln("connection close ...")
...
func (f *FakeChannel) ExchangeDeclare(name, kind string, durable, autoDelete, internal, noWait bool, args amqp.Table) error {
	logrus.Debugf("exchange with name %s and kind %s created", name, kind)
...
func (f *FakeChannel) QueueDeclare(name string, durable, autoDelete, exclusive, noWait bool, args amqp.Table) (amqp.Queue, error) {
	logrus.Debugf("queue %s declared", name)
...
func (f *FakeChannel) Qos(prefetchCount, prefetchSize int, global bool) error {
	logrus.Debugf("Qos controls with prefetch count %d and prefetch size %d and global %v add.", prefetchCount, prefetchSize, global)
...
func (f *FakeChannel) QueueBind(name, key, exchange string, noWait bool, args amqp.Table) error {
	logrus.Debugf("queue with name %s and key %s bind to exchange %s", name, key, exchange)
...
func (f *FakeChannel) Cancel(consumer string, noWait bool) error {
	logrus.Debugf("cunsumer %s cancel", consumer)
